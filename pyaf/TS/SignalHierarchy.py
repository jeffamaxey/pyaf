...
class cSignalHierarchy:
    ...
    def to_dict(self, iWithOptions = False):
        lDict = {
            'Structure': self.mStructure,
            'Models': self.mModels.to_dict(iWithOptions=False),
        };
        if(iWithOptions):
    ...
    def create_HierarchicalStructure(self):
        ...
        self.mStructure = {level: {} for level in range(lLevelCount)}
    ...
    def checkData(self , df):
        if (self.mHorizon != int(self.mHorizon)):
            raise tsutil.PyAF_Error(f"PYAF_ERROR_NON_INTEGER_HORIZON {str(self.mHorizon)}");
        if (self.mHorizon < 1):
            raise tsutil.PyAF_Error(
                f"PYAF_ERROR_NEGATIVE_OR_NULL_HORIZON {str(self.mHorizon)}"
            );
        if (self.mDateColumn not in df.columns):
            raise tsutil.PyAF_Error(
                f"PYAF_ERROR_HIERARCHY_TIME_COLUMN_NOT_FOUND {str(self.mDateColumn)}"
            );
        type1 = df[self.mDateColumn].dtype
        # print(type1)
        if type1.kind not in ['M', 'i', 'u', 'f']:
            raise tsutil.PyAF_Error(
                f"PYAF_ERROR_TIME_COLUMN_TYPE_NOT_ALLOWED '{str(self.mDateColumn)}' '{str(type1)}'"
            );
        # level 0 is the original/physical columns
        for k in self.mStructure[0]:
            if (k not in df.columns):
                raise tsutil.PyAF_Error(f"PYAF_ERROR_HIERARCHY_BASE_COLUMN_NOT_FOUND {str(k)}");
            # print(type2)
            type2 = df[k].dtype
            if type2.kind not in ['i', 'u', 'f']:
                raise tsutil.PyAF_Error(
                    f"PYAF_ERROR_HIERARCHY_BASE_SIGNAL_COLUMN_TYPE_NOT_ALLOWED '{str(k)}' '{str(type2)}'"
                );
    ...
    def create_all_levels_dataset(self, df):
        ...
        lMapped = all(k in df.columns for k in self.mStructure[0])
        if(not lMapped):
            i = 0;
            for k in self.mStructure[0]:
    ...
    def addVars(self, df):
        return self.create_all_levels_dataset(df)
    ...
    def get_plot_annotations(self):
        ...
        if lHasModels:
            ...
            for level in sorted(self.mStructure.keys()):
                for signal in sorted(self.mStructure[level].keys()):
                    lEngine = self.mModels
                    lMAPE = 'MAPE = %.4f' % self.mValidPerfs[f"{str(signal)}_Forecast"].mMAPE
                    lReconciledMAPEs = [ ]
                    for lPrefix in lPrefixes:
                        lMAPE_Rec = self.mValidPerfs[f"{str(signal)}_" + lPrefix + "_Forecast"].mMAPE
                    ...
                    for col1 in sorted(self.mStructure[level][signal]):
                        ...
                        lAnnotations[f"{str(signal)}_" + col1] = ('%2.2f %%' % lProp)
        return lAnnotations
    ...
    def plot_as_png_base64(self , name = None):
        ...
        lAnnotations = self.get_plot_annotations()
        return tsplot.plot_hierarchy_as_png_base64(self.mStructure, lAnnotations, name)
    ...
    def forecastAllModels_with_one_engine(self, iAllLevelsDataset, H, iDateColumn):
        ...
        for signal in lEngine.mSignalDecomposition.mSignals:
            ...
            lSigColumns = lSigColumns + [
                signal,
                f'{str(signal)}_Forecast',
                f'{str(signal)}_Forecast_Lower_Bound',
                f'{str(signal)}_Forecast_Upper_Bound',
            ]
        ...
        if (self.discard_nans_in_aggregate_signals()):
            ...
            lForecast_DF[f'{str(signal)}_Forecast'] = lForecast_DF[
                f'{str(signal)}_Forecast'
            ].fillna(0.0)
    ...
    def getEstimPart(self, df):
        lEngine = self.mModels
        return (
            lEngine.mSignalDecomposition.mBestModel.mTimeInfo.mSplit.getEstimPart(
                df
            )
        )
    ...
    def getValidPart(self, df):
        lEngine = self.mModels
        return (
            lEngine.mSignalDecomposition.mBestModel.mTimeInfo.mSplit.getValidPart(
                df
            )
        )
    ...
    def computeBottomUpForecast(self, iForecast_DF, level, signal, iPrefix = "BU"):
        ...
        if (new_BU_forecast is None):
            new_BU_forecast = iForecast_DF[f"{str(signal)}_Forecast"];
    ...
    def computeBottomUpForecasts(self, iForecast_DF):
        ...
        for level in sorted(self.mStructure.keys()):
            for signal in sorted(self.mStructure[level].keys()):
                ...
                lForecast_DF_BU[f"{str(signal)}_BU_Forecast"] = new_BU_forecast;
    ...
    def get_clean_signal_and_forecasts(self, iForecast_DF, signal, iPrefixes):
        lEngine = self.mModels
        lForecasts = [f"{str(signal)}_Forecast"]
        lForecasts += [
            f"{str(signal)}_" + lPrefix + "_Forecast" for lPrefix in iPrefixes
        ]
        lColumns = [lEngine.mSignalDecomposition.mDateColumns[signal] , signal ] + lForecasts
        return iForecast_DF[lColumns]
    ...
    def computePerfOnCombinedForecasts(self, iForecast_DF):
        ...
        self.mValidPerfs = {}
        lPerfs = {};
        logger.info(f"STRUCTURE {str(sorted(list(self.mStructure.keys())))}");
        logger.info(f"DATASET_COLUMNS {str(iForecast_DF.columns)}");
        for level in sorted(self.mStructure.keys()):
            logger.info(
                f"STRUCTURE_LEVEL {(level, sorted(list(self.mStructure[level].keys())))}"
            );
            for signal in sorted(self.mStructure[level].keys()):
                ...
                lColumns = [signal, f"{str(signal)}_Forecast"] + [
                    f"{str(signal)}_" + lPrefix + "_Forecast"
                    for lPrefix in lPrefixes
                ]
                ...
                lPerfFit = lEngine.computePerf(
                    lFrameFit[signal], lFrameFit[f"{str(signal)}_Forecast"], signal
                )
                lPerfValid = lEngine.computePerf(
                    lFrameValid[signal],
                    lFrameValid[f"{str(signal)}_Forecast"],
                    signal,
                )
                self.mEstimPerfs[f"{str(signal)}_Forecast"] = lPerfFit
                self.mValidPerfs[f"{str(signal)}_Forecast"] = lPerfValid
                for iPrefix in lPrefixes:
                    lName = f"{str(signal)}_" + iPrefix + "_Forecast"
        ...
        for (sig , perf) in sorted(lPerfs.items()):
            logger.info(f"REPORT_COMBINED_FORECASTS_FIT_PERF {str(perf[2].to_dict())}")
            logger.info(f"REPORT_COMBINED_FORECASTS_VALID_PERF {str(perf[3].to_dict())}")
        return lPerfs;
    ...
    def computeTopDownForecasts(self, iForecast_DF , iProp , iPrefix):
        ...
        for signal in sorted(self.mStructure[lHighestLevel].keys()):
            lForecast_DF_TD[
                f"{str(signal)}_" + iPrefix + "_Forecast"
            ] = iForecast_DF[f"{str(signal)}_Forecast"];
        for level in lLevelsReversed:
            for signal in sorted(self.mStructure[level].keys()):
                for col in sorted(self.mStructure[level][signal]):
                    new_TD_forecast = (
                        lForecast_DF_TD[f"{str(signal)}_" + iPrefix + "_Forecast"]
                        * iProp[signal][col]
                    );
                    lForecast_DF_TD[f"{str(col)}_" + iPrefix + "_Forecast"] = new_TD_forecast;
    ...
    def computeMiddleOutForecasts(self, iForecast_DF , iProp, iPrefix):
        ...
        for level in levels_below:
            for signal in sorted(self.mStructure[level].keys()):
                for col in sorted(self.mStructure[level][signal]):
                    new_MO_forecast = (
                        lForecast_DF_MO[f"{str(signal)}_" + iPrefix + "_Forecast"]
                        * iProp[signal][col]
                    );
                    lForecast_DF_MO[f"{str(col)}_" + iPrefix + "_Forecast"] = new_MO_forecast;
        ...
        for level in range(lMidLevel + 1 , len(self.mStructure.keys())):
            for signal in sorted(self.mStructure[level].keys()):
                ...
                lForecast_DF_MO[f"{str(signal)}_" + iPrefix + "_Forecast"] = new_MO_forecast;
    ...
    def computeOptimalCombination(self, iForecast_DF):
        ...
        lBaseNames = [];
        for level in sorted(self.mStructure.keys()):
            lBaseNames.extend(iter(sorted(self.mStructure[level].keys())))
        lBaseForecastNames = [f"{str(col)}_Forecast" for col in lBaseNames]
        ...
        lOptimalNames = [f"{str(col)}_OC_Forecast" for col in lBaseNames];
        df = pd.DataFrame(lOptimalForecasts.T);
        df.columns = lOptimalNames;
        return pd.concat([iForecast_DF , df] , axis = 1)
    ...
    def internal_forecast(self , iInputDS, iHorizon):
        ...
        logger = tsutil.get_pyaf_hierarchical_logger();
        logger.info(
            f"FORECASTING_HIERARCHICAL_MODEL_COMBINATION_METHODS {str(lCombinationMethods)}"
        );
    ...
    def forecast(self , iInputDS, iHorizon):
        ...
        return self.internal_forecast(iInputDS , iHorizon)
