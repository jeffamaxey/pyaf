...
for subdir1 in sorted(subdirs):
    ...
    test_target = "";
    for filename in sorted(glob.glob(f"{subdir1}/*/*/*/*.py")):
        bn = os.path.basename(filename);
        dirnames = os.path.dirname(filename).split("/");
        logdir = f"tests/references/artificial/{dirnames[2]}/{dirnames[3]}/{dirnames[4]}"
        mkdir_p(logdir)
        logname = bn.replace("/" , "_");
        logname = logname.replace(".py" , ".log");

        logfile = f"logs/{logname}";
        reflogfile = f"{logdir}/{logname}";
